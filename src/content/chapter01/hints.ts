...
export const HINTS: Record<string, HintEntry> = {
  ...
  hidden_restore: {
    ...
    title: '壁の四つの空白',
    steps: [
      '背景幕の裏の年代記には、写真を抜き取った跡が四つある。四枚は館内の四か所に分けて隠されている。',
      '三か所はそのまま持ち出せる。受付の額だけは違う。台紙の裏に何かある気配はあるが、この額が何を隠しているのかを先に確かめないと開けられない。写真といまの部屋を見比べる謎が、そのまま鍵になっている。',
      '暗室の作業台の下の棚、事務机の抽斗の底、金庫の中の三枚は、そのまま手に取れる。受付の額の一枚は、写真といまの撮影室の違いを三つとも見つけたあとで台紙が起こせるようになる。四枚を年代記の空白へ戻す。',
    ],
  },
}
